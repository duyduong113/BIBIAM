...
namespace BIBIAM.Core.Data.Providers
{
    public class SqlHelper
    {
        ...
        public int ExecuteNoneQuery(string spName, List<SqlParameter> listpara, IDbConnection con, IDbTransaction trans)
        {
            int n = -1;
            try
            {
                var adoNetConn = ((IHasDbConnection)con).DbConnection;
                //SqlCommand command = new SqlCommand(spName, (SqlConnection)adoNetConn);
                SqlCommand command = new SqlCommand(spName, (SqlConnection)trans.Connection, (SqlTransaction)trans);
                command.CommandType = CommandType.StoredProcedure;
                command.CommandTimeout = 0;
                if (listpara != null)
                {
