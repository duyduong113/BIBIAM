...
namespace BIBIAM.Core.Data.DataObject
{
    public class Merchant_Info_DAO
    {
        ...
        public string ChangeStatusAuth(string[] ids, string connectionString, string Username)
        {
            using (var db = new OrmliteConnection().openConn(connectionString))
            {
                try
                {
                    ...
                    foreach (var id in ids)
                    {
                        ...
                        if (checkID != null)
                        {
                            checkID.trang_thai_xac_thuc = (checkID.trang_thai_xac_thuc == AllConstant.trang_thai_xac_thuc.DA_XAC_THUC) ? AllConstant.trang_thai_xac_thuc.CHUA_XAC_THUC : AllConstant.trang_thai_xac_thuc.DA_XAC_THUC;
                            checkID.nguoi_cap_nhat = Username;
                            checkID.ngay_cap_nhat = DateTime.Now;
