...
AS
BEGIN
	if not exists (select * from OPENQUERY(MYSQL_BIBIAM_DEV,'SELECT ma_banner FROM bibiam.banner') where ma_banner=@ma_banner)			    
	begin
			--Insert Banner
			INSERT INTO openquery(MYSQL_BIBIAM_DEV,'SELECT ma_banner,ma_chuyen_muc,image_link,url_link,image,loai,vi_tri,trang_thai FROM banner')
