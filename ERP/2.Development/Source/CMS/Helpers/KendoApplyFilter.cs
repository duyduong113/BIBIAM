...
namespace CMS.Helpers
{
    public static class KendoApplyFilter
    {
            ...
            using (var dbCommand = dbConn.CreateCommand())
            {
                ...
                if (request.Filters.Any())
                {
                    ...
                    if (request.PageSize > 0)
                    {
                        ...
                        if (request.Page > 1)
                        {
                            var sql = dbCommand.CommandText.Replace(" )", " " + " WHERE" + where + ")").Replace("ORDER BY \"id\"", (!string.IsNullOrEmpty(order.Trim()) ? order : "ORDER BY (SELECT NULL)"));
                ...
                else
                {
                    ...
                    if (request.PageSize > 0)
                    {
                        data.Data = dbCommand.Select<T>(q => q.Select("SELECT * FROM " + Tname + where).Limit(skip: (request.Page - 1) * request.PageSize, rows: request.PageSize));
                        if (request.Page > 1)
                        {
                            var sql = dbCommand.CommandText.Replace("ORDER BY \"id\"", (!string.IsNullOrEmpty(where.Trim()) ? where : "ORDER BY (SELECT NULL)"));
                            data.Data = dbConn.Select<T>(sql);
                        }
                    }
                    else
        ...
        public static DataSourceResult KendoDataExclude<T>([DataSourceRequest]DataSourceRequest request, List<string> exclude)
        {
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbCommand = dbConn.CreateCommand())
            {
                ...
                if (request.Filters.Any())
                {
                    ...
                    var where = KendoApplyFilter.ApplyFilterExclude(request.Filters[0], "", exclude);
                    var order = (!String.IsNullOrEmpty(sortString) ? (" Order By " + sortString) : "");
                    if (request.PageSize > 0)
                    {
                        data.Data = dbCommand.Select<T>(q => q.Select("SELECT * FROM " + Tname + " WHERE" + where + order).Limit(skip: (request.Page - 1) * request.PageSize, rows: request.PageSize));
                        if (request.Page > 1)
                        {
                            var sql = dbCommand.CommandText.Replace(" )", " " + " WHERE" + where + ")").Replace("ORDER BY \"id\"", order);
                            data.Data = dbConn.Select<T>(sql);
                        }
                    }
                    else
                ...
                else
                {
                    ...
                    if (request.PageSize > 0)
                    {
                        data.Data = dbCommand.Select<T>(q => q.Select("SELECT * FROM " + Tname + order).Limit(skip: (request.Page - 1) * request.PageSize, rows: request.PageSize));
                        if (request.Page > 1)
                        {
                            var sql = dbCommand.CommandText.Replace("ORDER BY \"id\"", (!string.IsNullOrEmpty(order.Trim()) ? order : "ORDER BY (SELECT NULL)"));
                            data.Data = dbConn.Select<T>(sql);
                        }
                    }
                    else
                    {
                        data.Data = dbConn.Select<T>(q => q.Select("SELECT * FROM " + Tname + order));
                    }
                    data.Total = dbConn.Scalar<int>("SELECT COUNT(*) FROM " + Tname);
                }
                return data;
                ...
                else
                {
                    ...
                    if (request.PageSize > 0)
                    {
                        try
                        {
                            ...
                            if (request.Page > 1)
                            {
                                var sql = dbCommand.CommandText.Replace(" )", " " + where + ")").Replace("ORDER BY \"id\"", (!string.IsNullOrEmpty(order.Trim()) ? order : "ORDER BY (SELECT NULL)"));
                                data.Data = dbConn.Select<T>(sql);
                            }
                        }
                        catch (Exception ex)
