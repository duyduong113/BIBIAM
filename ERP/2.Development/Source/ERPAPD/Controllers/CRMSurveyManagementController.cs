...
{
    [Authorize]
    public class CRMSurveyManagementController : CustomController
    {
        //
        ...
        public ActionResult Index()
        {
            ...
            //{
            //    OrmLiteConfig.DialectProvider.UseUnicode = true;
            //    dbConn.DropTables(typeof(CRM_Survey_Management_Customer));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(CRM_Survey_Management_Customer));
            //}
            //using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            //{
            //    OrmLiteConfig.DialectProvider.UseUnicode = true;
            //    dbConn.DropTables(typeof(CRM_Survey_Management_Proceeded), typeof(CRM_Survey_Management_Proceeded_Answer));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(CRM_Survey_Management_Proceeded), typeof(CRM_Survey_Management_Proceeded_Answer));
            //}
            //using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            //{
            //    OrmLiteConfig.DialectProvider.UseUnicode = true;
            //    dbConn.DropTables(typeof(CRM_Survey_Management_User));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(CRM_Survey_Management_User));
            //}
            //using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            //{
            //    OrmLiteConfig.DialectProvider.UseUnicode = true;
            //    dbConn.DropTables(typeof(CRM_Survey_Management_Skip));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(CRM_Survey_Management_Skip));
            //}
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                if (asset.View)
                {
        ...
        public ActionResult Question_Read([DataSourceRequest] DataSourceRequest request, string SurveyManagementID)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    string query = @"SELECT q.Id, q.QuestionID,q.[Description],mq.SortOrder FROM CRM_Survey_Management_Question mq
            ...
            if (asset.Update && ModelState.IsValid)
            {
                ...
                using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                {
                    try
                    {
                        ...
                        var currentUserlist = dbConn.Select<CRM_Survey_Management_User>("SurveyManagementID={0}", model.SurveyManagementID);
                        //check question neu la all
