    ...
    public class DistrictController : CustomController
    {
        ...
        public ActionResult District_Read([DataSourceRequest] DataSourceRequest request)
        {
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            {
                ...
                var data = new List<DC_Location_District>();
                if (request.Filters.Any())
                {
                    var where = KendoApplyFilter.ApplyFilter(request.Filters[0], "data.");


                    data = DC_Location_District.GetDC_Location_Districts(where, "DistrictID DESC"); 
                }
                else
                {
                    data = DC_Location_District.GetDC_Location_Districts("1=1", "DistrictID DESC"); 
                }
        ...
        public ActionResult District_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Location_District> listEx)
        {
            if (asset.Create)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                    ...
                    try
                    {
                        if (listEx != null)
                        {
                            foreach (var regis in listEx)
                            {
                                ...
                                string id = "";
                                var write = new DC_Location_District();
                                var checkID = dbConn.Select<DC_Location_District>().OrderByDescending(m => m.RowCreatedTime).FirstOrDefault();
                                if (checkID != null)
                                {
                                ...
                                var check = dbConn.FirstOrDefault<DC_Location_District>("DistrictName COLLATE Latin1_General_CI_AI LIKE {0} AND CityID = {1}", regis.DistrictName, regis.CityID);
                                if (check != null)
                                {
        ...
        public ActionResult District_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Location_District> listEx)
        {
            if (asset.Update)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                    ...
                    try
                    {
                        if (listEx != null && ModelState.IsValid)
                        {
                            foreach (var regis in listEx)
                            {
                                ...
                                //    return Json(listEx.ToDataSourceResult(request, ModelState));
                                //}
                                var write = dbConn.FirstOrDefault<DC_Location_District>("DistrictID={0}", regis.DistrictID);
                                var check = dbConn.FirstOrDefault<DC_Location_District>("DistrictName COLLATE Latin1_General_CI_AI LIKE {0} AND CityID = {1}", regis.DistrictName, regis.CityID);
                                if (check != null)
                                {
                                    ModelState.AddModelError("", " District Name  is exists.");
            ...
            if (asset.Delete)
            {
                try
                {
                    using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                    {
                        ...
                        {
                            var check = dbConn.FirstOrDefault<DC_Location_District>("DistrictID={0}", item);
                            dbConn.Delete(check);
                        }
                    }
                }
                ...
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    if (Request.Files["FileUpload"] != null && Request.Files["FileUpload"].ContentLength > 0)
                    {
                        ...
                        if (fileExtension == ".xlsx")
                        {
                            ...
                            FileInfo template = new FileInfo(Server.MapPath(@"~\ExportExcelFile\DC_Location_District.xlsx"));
                            template.CopyTo(errorFileLocation);
                            FileInfo _fileInfo = new FileInfo(errorFileLocation);
                            var _excelPkg = new ExcelPackage(_fileInfo);

                            ExcelWorksheet oSheet = excelPkg.Workbook.Worksheets["DC_Location_District"];
                            ExcelWorksheet eSheet = _excelPkg.Workbook.Worksheets["DC_Location_District"];
                            ...
                            for (int i = 2; i <= totalRows; i++)
                            {
                                ...
                                {
                                    var write = new DC_Location_District();
                                    var check = dbConn.FirstOrDefault<DC_Location_District>("DistrictName COLLATE Latin1_General_CI_AI LIKE {0} AND CityID = {1}", DistrictName, id_city);
                                    var checkCity = dbConn.FirstOrDefault<DC_Location_District>("CityID={0}", id_city);
