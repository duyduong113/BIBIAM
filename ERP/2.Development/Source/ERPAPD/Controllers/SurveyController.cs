...
{
    [Authorize]
    public class SurveyController : CustomController
    {
        public ActionResult Index()
        {
            ...
            //    dbConn.DropTables(typeof(DC_Survey_Category), typeof(DC_Survey_Question), typeof(DC_Survey_AnswerList), typeof(DC_Survey_AnswerConfig));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(DC_Survey_Category), typeof(DC_Survey_Question), typeof(DC_Survey_AnswerList), typeof(DC_Survey_AnswerConfig));
            //}
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                if (asset.View)
                {
        ...
        public ActionResult SurveyCategory_Read([DataSourceRequest] DataSourceRequest request)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    return Json(KendoApplyFilter.KendoData<DC_Survey_Category>(request));
                }
            }
            else
            {
        ...
        public ActionResult SurveyCategory_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_Category> list)
        {
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Create)
                {
                    try
                    {
                        ...
                        {
                            var data = new DC_Survey_Category();
                            foreach (var item in list)
                            {
                                ...
                                var checkID = dbConn.Select<DC_Survey_Category>("SELECT CategoryID, Id FROM dbo.DC_Survey_Category ORDER BY Id DESC").FirstOrDefault();
                                if (checkID != null)
                                {
                                ...
                                var checkColumName = dbConn.Select<DC_Survey_Category>("SELECT Name FROM dbo.DC_Survey_Category WHERE Name = '" + item.Name + "'").FirstOrDefault();
        ...
        public ActionResult SurveyCategory_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_Category> list)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        {
                            var check = dbConn.GetFirstColumn<string>("Select CategoryID FROM DC_Survey_Question WHERE CategoryID=" + item);
                            if (check.Count > 0)
                            {
                                dbTrans.Rollback();
                            ...
                            dbConn.Delete<DC_Survey_Category>(where: "CategoryID={0}".Params(item));
                        }
                        dbTrans.Commit();
                    }
                    catch (Exception ex)
        ...
        public ActionResult ExportCategory([DataSourceRequest] DataSourceRequest request)
        {
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                if (asset.Export)
                {
                    var data = dbConn.Select<DC_Survey_Category>();
                    IEnumerable datas = data.ToDataSourceResult(request).Data;

                    //using (ExcelPackage excelPkg = new ExcelPackage())
                    FileInfo fileInfo = new FileInfo(Server.MapPath(@"~\ExportExcelFile\DC_SurveyCategory.xlsx"));
                    var excelPkg = new ExcelPackage(fileInfo);
                    ExcelWorksheet dataSheet = excelPkg.Workbook.Worksheets["DC_SurveyCategory"];
                    int rowData = 1;
                    foreach (DC_Survey_Category item in datas)
                    {
                        int i = 1;
                        rowData++;
                        ...
                        if (item.RowCreatedTime.ToString("dd/MM/yyyy") != "01/01/1900")
                        {
                            dataSheet.Cells[rowData, i++].Value = item.RowCreatedTime.ToString();
                        }
                        else
                        {
                        ...
                        if (item.RowLastUpdatedTime.ToString("dd/MM/yyyy") != "01/01/1900")
                        {
                            dataSheet.Cells[rowData, i++].Value = item.RowLastUpdatedTime.ToString();
                        }
                        else
                        {
                            dataSheet.Cells[rowData, i++].Value = "";
                    ...
                    var dataCategory = dbConn.Select<DC_Survey_Category>("SELECT * FROM DC_Survey_Category");
                    ...
                    foreach (DC_Survey_Category item1 in dataCategory)
                    {
                        int i = 1;
                        rowParentCategory++;
                    ...
                    string fileName = "DC_SurveyCategory_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xlsx";
                ...
                else
                {
                    ...
                    return File("", //The binary data of the XLS file
                        ...
                        "DC_SurveyCategory_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xls");     //Suggested file name in the "Save as" dialog which will be displayed to the end user
                }
        }
                ...
                if (Request.Files["FileUpload"] != null && Request.Files["FileUpload"].ContentLength > 0)
                {
                    ...
                    if (fileExtension == ".xlsx")
                    {
                        ...
                        using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                        {
                            ...
                            var excelPkg = new ExcelPackage(fileInfo);
                            FileInfo template = new FileInfo(Server.MapPath(@"~\ExportExcelFile\DC_SurveyCategory.xlsx"));
                            template.CopyTo(errorFileLocation);
                            FileInfo _fileInfo = new FileInfo(errorFileLocation);
                            var _excelPkg = new ExcelPackage(_fileInfo);
                            ExcelWorksheet oSheet = excelPkg.Workbook.Worksheets["DC_SurveyCategory"];
                            ExcelWorksheet eSheet = _excelPkg.Workbook.Worksheets["DC_SurveyCategory"];
                            ...
                            for (int i = 2; i <= totalRows; i++)
                            {
                                ...
                                {
                                    var data = new DC_Survey_Category();
                                    var check = dbConn.Select<DC_Survey_Category>("SELECT * FROM DC_Survey_Category WHERE CategoryID = '" + Id + "'").FirstOrDefault();
                                    var parentId = dbConn.Select<DC_Survey_Category>("SELECT * FROM DC_Survey_Category WHERE Name = N'" + Parent + "'").FirstOrDefault();
        ...
        public ActionResult Question_Read([DataSourceRequest] DataSourceRequest request)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    return Json(KendoApplyFilter.KendoData<DC_Survey_Question>(request));
                }
            }
            else
            {
        ...
        public ActionResult Question_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_Question> list)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Create)
                {
                    try
                    {
                        if (list != null && ModelState.IsValid)
                        {
                            var data = new DC_Survey_Question();
                            foreach (var item in list)
                            {
                                string id = "";
                                ...
                                var checkID = dbConn.Select<DC_Survey_Question>("SELECT QuestionID, Id FROM dbo.DC_Survey_Question ORDER BY Id DESC").FirstOrDefault();
                                if (checkID != null)
                                {
                                ...
                                var checkColumName = dbConn.Select<DC_Survey_Question>("SELECT [Description], Id FROM dbo.DC_Survey_Question WHERE [Description] = N'" + item.Description + "' AND [Type] = '" + item.Type + "' AND CategoryID ='" + item.CategoryID + "'").FirstOrDefault();
                                if (checkColumName != null)
                                {
        ...
        public ActionResult Question_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_Question> list)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
                    ...
                    try
                    {
                        if (list != null && ModelState.IsValid)
                        {
                            foreach (var item in list)
                            {
                                ...
                                item.RowLastUpdatedUser = currentUser.UserName;
                                dbConn.Update(item);
                                var checkConfig = dbConn.Select<DC_Survey_AnswerConfig>("select Id from DC_Survey_AnswerConfig where QuestionID = '" + item.QuestionID + "'").FirstOrDefault();
                                if (checkConfig != null && item.Type != "Multi choice")
                                {
                                    dbConn.Delete<DC_Survey_AnswerConfig>(where: "QuestionID={0}".Params(item.QuestionID));
                                }
                                else if (checkConfig == null && item.Type == "Multi choice")
                                {
                                    var data2 = new DC_Survey_AnswerConfig();
                                    data2.QuestionID = item.QuestionID;
                                    //data2.AnswerID = id;
                                    data2.Description = "Limit Choices Quantity";
                                    data2.Detail = 0;
                                    data2.Active = true;
                                    data2.RowCreatedTime = DateTime.Now;
                                    data2.RowCreatedUser = currentUser.UserName;
                                    data2.RowLastUpdatedTime = DateTime.Parse("1900-01-01");
                                    data2.RowLastUpdatedUser = "";
                                    dbConn.Save(data2);
                                }
                            }
                            dbTrans.Commit();
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        foreach (var item in listdata)
                        {
                            var checkexits = dbConn.GetFirstColumn<string>("SELECT QuestionID FROM DC_Survey_Management_Question WHERE QuestionID= '" + item + "'");
                            if (checkexits.Count > 0)
                            {
                                dbTrans.Rollback();
                                return Json(new { success = false, alert = "Câu hỏi này không thể xóa vì đã sử dụng trong survey." });
                            }
                            dbConn.Delete<DC_Survey_AnswerList>(where: "QuestionID={0}".Params(item));
                            dbConn.Delete<DC_Survey_AnswerConfig>(where: "QuestionID={0}".Params(item));
                            dbConn.Delete<DC_Survey_Question>(where: "QuestionID={0}".Params(item));
                        }
                        dbTrans.Commit();
                        return Json(new { success = true });
                    }
        ...
        public ActionResult ExportQuestion([DataSourceRequest] DataSourceRequest request)
        {
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                if (asset.Export)
                {
                    var data = dbConn.Select<DC_Survey_Question>("SELECT q.*, c.Name FROM DC_Survey_Question q left join DC_Survey_Category c on q.CategoryID = c.CategoryID");
                    IEnumerable datas = data.ToDataSourceResult(request).Data;

                    //using (ExcelPackage excelPkg = new ExcelPackage())
                    FileInfo fileInfo = new FileInfo(Server.MapPath(@"~\ExportExcelFile\DC_SurveyQuestion.xlsx"));
                    var excelPkg = new ExcelPackage(fileInfo);
                    ExcelWorksheet dataSheet = excelPkg.Workbook.Worksheets["DC_SurveyQuestion"];
                    int rowData = 1;
                    foreach (DC_Survey_Question item in datas)
                    {
                        int i = 1;
                        rowData++;
                    ...
                    MemoryStream output = new MemoryStream();
                    excelPkg.SaveAs(output);
                    string fileName = "DC_SurveyQuestion_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xlsx";
                    string contentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                ...
                else
                {
                    ...
                    return File("", //The binary data of the XLS file
                        "application/vnd.ms-excel", //MIME type of Excel files
                        "DC_SurveyQuestion_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xls");     //Suggested file name in the "Save as" dialog which will be displayed to the end user
                }
        }
                ...
                if (Request.Files["FileUpload"] != null && Request.Files["FileUpload"].ContentLength > 0)
                {
                    ...
                    if (fileExtension == ".xlsx")
                    {
                        ...
                        using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                        {
                            ...
                            FileInfo fileInfo = new FileInfo(fileLocation);
                            var excelPkg = new ExcelPackage(fileInfo);
                            FileInfo template = new FileInfo(Server.MapPath(@"~\ExportExcelFile\DC_SurveyQuestion.xlsx"));
                            template.CopyTo(errorFileLocation);
                            FileInfo _fileInfo = new FileInfo(errorFileLocation);
                            var _excelPkg = new ExcelPackage(_fileInfo);
                            ExcelWorksheet oSheet = excelPkg.Workbook.Worksheets["DC_SurveyQuestion"];
                            ExcelWorksheet eSheet = _excelPkg.Workbook.Worksheets["DC_SurveyQuestion"];
                            int totalRows = oSheet.Dimension.End.Row;
                            for (int i = 2; i <= totalRows; i++)
                            {
                                string Id = oSheet.Cells[i, 1].Value != null ? oSheet.Cells[i, 1].Value.ToString() : "";
                                ...
                                {
                                    var data = new DC_Survey_Question();
                                    var check = dbConn.Select<DC_Survey_Question>("SELECT * FROM DC_Survey_Question WHERE QuestionID = '" + Id + "'").FirstOrDefault();
                                    //var parentId = dbConn.Select<DC_Survey_Category>("SELECT * FROM DC_Survey_Category WHERE Name = N'" + Parent + "'").FirstOrDefault();
        ...
        public ActionResult AnswerList_Read([DataSourceRequest] DataSourceRequest request, string QuestionID)
        {
            ...
            return Json(KendoApplyFilter.KendoData<DC_Survey_AnswerList>(request, "QuestionID='" + QuestionID + "'"));
        }
        public ActionResult AnswerList_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")] IEnumerable<DC_Survey_AnswerList> list, string QuestionID)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Create)
                {
                    try
                    {
                        ...
                        {
                            var data2 = new DC_Survey_AnswerConfig();
                            var checkType = dbConn.Select<DC_Survey_Question>("select [Type] from DC_Survey_Question where QuestionID ='" + QuestionID + "'").FirstOrDefault();
                            var checkConfig = dbConn.Select<DC_Survey_AnswerConfig>("select * from DC_Survey_AnswerConfig where QuestionID = '" + QuestionID + "'").FirstOrDefault();
                            var data = new DC_Survey_AnswerList();
                            foreach (var item in list)
                            {
                                string id = "";
                                ...
                                var checkID = dbConn.Select<DC_Survey_AnswerList>("SELECT AnswerID, Id FROM dbo.DC_Survey_AnswerList ORDER BY Id DESC").FirstOrDefault();
                                if (checkID != null)
                                {
                                ...
                                //var checkColumName = dbConn.Select<DC_Survey_Question>("SELECT [Description] FROM dbo.DC_Survey_Question WHERE [Description] = '" + item.Description + "'").FirstOrDefault();
                                //if (checkColumName != null)
                                //{
                                //    ModelState.AddModelError("", "Question is exists");
                                //    return Json(list.ToDataSourceResult(request, ModelState));
        ...
        public ActionResult AnswerList_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_AnswerList> list, string QuestionID)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        var listdata = data.Split(separators, StringSplitOptions.RemoveEmptyEntries);
                        //check
                        var currentQuestionID = dbConn.Scalar<string>("SELECT QuestionID FROM DC_Survey_AnswerList WHERE AnswerID= {0}", listdata[0]);
                        var checkexits = dbConn.GetFirstColumn<string>("SELECT QuestionID FROM DC_Survey_Management_Question WHERE QuestionID= '" + currentQuestionID + "'");
                        if (checkexits.Count > 0)
                        {
                            dbTrans.Rollback();
                        ...
                        foreach (var item in listdata)
                        {
                            dbConn.Delete<DC_Survey_AnswerList>(where: "AnswerID={0}".Params(item));
                        }
                        dbTrans.Commit();
                    }
                    catch (Exception ex)
        ...
        {
            var dbConn = Helpers.OrmliteConnection.openConn();
            return Json(KendoApplyFilter.KendoData<DC_Survey_AnswerConfig>(request, "QuestionID='" + QuestionID + "'"));
        }
        public ActionResult Configuration_Update([DataSourceRequest] DataSourceRequest request, DC_Survey_AnswerConfig config, string QuestionID)
        {
            IEnumerable<DC_Survey_AnswerConfig> u = new DC_Survey_AnswerConfig[] { };
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
                {
                    try
                    {
                        var check = dbConn.Select<DC_Survey_AnswerList>("select * from DC_Survey_AnswerList where QuestionID='" + QuestionID + "'");
                        ...
                        dbConn.Update<DC_Survey_AnswerConfig>(set: "Detail={0}, Active ={1}, RowLastUpdatedUser={2}, RowLastUpdatedTime={3}".Params(config.Detail, config.Active, currentUser.UserName, DateTime.Now), where: "AnswerID={0}".Params(config.AnswerID));
                        dbTrans.Commit();
                    }
        ...
        {
            var dbConn = Helpers.OrmliteConnection.openConn();
            var data = dbConn.Select<DC_Survey_Category>("select * from DC_Survey_Category");
            return Json(data);
        }
    }
}
