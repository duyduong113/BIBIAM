...
{
    [Authorize]
    public class CRMProductPriceController : CustomController
    {
        // GET: CRMProductPrice
            ...
            if (asset.View)
            {
                using (IDbConnection dbConn = ERPAPD.Helpers.OrmliteConnection.openConn())
                {
                    try
                    {
                        ...
                        ViewBag.listProduct = dbConn.Select<CRM_Product>();
                    }
                    catch (Exception) { }
        ...
        public ActionResult Read([DataSourceRequest]DataSourceRequest request)
        {
            using (IDbConnection dbConn = OrmliteConnection.openConn())
            {
                ...
                if (asset.View)
                {
                    string strQuery = @"SELECT p.* FROM [CRM_Product_Price] p";
                    data = KendoApplyFilter.KendoDataByQuery<CRM_Product_Price>(request, strQuery, "");
                    return Json(data);
                }
                return RedirectToAction("NoAccessRights", "Error");
            }
        }
        public ActionResult Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<CRM_Product_Price> listEx)
        {
            if (asset.Create)
            {
                using (IDbConnection dbConn = ERPAPD.Helpers.OrmliteConnection.openConn())
        ...
        public ActionResult Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<CRM_Product_Price> listEx)
        {
            if (asset.Update)
            {
                using (IDbConnection dbConn = ERPAPD.Helpers.OrmliteConnection.openConn())
            ...
            if (asset.Delete)
            {
                try
                {
                    using (IDbConnection dbConn = ERPAPD.Helpers.OrmliteConnection.openConn())
                    {
                        ...
                        {
                            var check = dbConn.FirstOrDefault<CRM_Product_Price>("Id={0}", item);
                            dbConn.Delete(check);
                        }
                    }
                }
