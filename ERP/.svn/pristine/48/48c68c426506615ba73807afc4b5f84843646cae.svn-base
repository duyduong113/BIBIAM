...
namespace MCC.Controllers
{
    ...
    public class Merchant_ReportPriceController : CustomController
    {
        ...
        public ActionResult GetProductInfo(string part_no)
        {
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            {
                var data = new Merchant_Product();
                if (isAdmin)
                    data = dbConn.FirstOrDefault<Merchant_Product>("part_no = {0}", part_no);
                else
                    data = dbConn.FirstOrDefault<Merchant_Product>("ma_gian_hang ={0} and part_no = {1}", currentUser.ma_gian_hang, part_no);
                if (data != null)
                {
