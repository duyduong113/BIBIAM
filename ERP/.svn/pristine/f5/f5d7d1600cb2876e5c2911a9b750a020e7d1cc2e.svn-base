...
{
    public class CustomerSupportManageInfoController : CustomController
    {
        ...
        public ActionResult Index()
        {
            ...
            //    dbConn.DropTables(typeof(DC_CS_ManageInfo), typeof(DC_CS_ManageInfo));
            //    const bool overwrite = true;
            //    dbConn.CreateTables(overwrite, typeof(DC_CS_ManageInfo), typeof(DC_CS_ManageInfo));
            //}
            if (asset.View)
            {
        ...
        public ActionResult Edit(int Id)
        {
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                ...
                if (asset.Update)
                {
                    ...
                    var hOld = dbConn.Select<DC_CS_ManageInfo>("SELECT * FROM DC_CS_ManageInfo Where ID = " + Id).FirstOrDefault();
                    if (hOld != null)
                    {
        ...
        [ValidateInput(false)]
        public ActionResult Create(DC_CS_ManageInfo newManageInfo)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        [ValidateInput(false)]
        public ActionResult Edit(DC_CS_ManageInfo article)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update && ModelState.IsValid)
                {
                    try
                    {
                        var CheckExit = dbConn.FirstOrDefault<DC_CS_ManageInfo>("ID =" + article.ID);
                        if (CheckExit == null)
                        {
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        var delete = new DC_CS_ManageInfo();
        ...
        public ActionResult DailyNew_Read([DataSourceRequest] DataSourceRequest request)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    return Json(KendoApplyFilter.KendoData<DC_CS_ManageInfo>(request));
                }
            }
            else
            {
        ...
        public ActionResult DailyNew_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_CS_ManageInfo> list)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Create)
        ...
        public ActionResult DailyNew_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_CS_ManageInfo> list)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
                {
                    try
                    {
                        ...
                        foreach (var item in listRowID)
                        {
                            dbConn.Update<DC_CS_ManageInfo>(set: "[IsActive]  = {0}".Params(false));
                        }
                        dbTrans.Commit();
                    }
                    catch (Exception e)
