...
begin
	...
	select p.ma_san_pham, p.ma_loai_san_pham, p.ma_gian_hang, p.ten_san_pham, p.part_no, p.mo_ta, p.noi_dung, p.trong_so, p.don_gia, p.gia_si, p.tu_khoa, p.tag,  REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(p.slug,CHAR(208), '-'),CHAR(10),'-'),CHAR(13),'-'),CHAR(13),'-'),CHAR(3),'-'),CHAR(' '),'-'),CHAR(5),'-') + '-' + CAST(p.id as varchar(10)), 
	p.url, p.xuat_xu, p.thuong_hieu, p.model, p.minimum_order, p.nguoi_xuat_ban, p.ngay_xuat_ban, p.trang_thai_xuat_ban, p.nguoi_duyet, p.ngay_duyet, p.trang_thai_duyet, p.trang_thai, p.ngay_tao, p.nguoi_tao, p.ngay_cap_nhat, p.nguoi_cap_nhat			
	FROM merchant_product AS p WITH (NOLOCK)
	LEFT JOIN OPENQUERY(MYSQL_BIBIAM_DEV, 'select * from merchant_product' ) AS p2
    ON p2.ma_san_pham = p.ma_san_pham and p2.ma_gian_hang = p.ma_gian_hang
	where p2.ma_gian_hang is null

	EXEC [p_BaselineMasterData_SyncToMySQL_All]
end