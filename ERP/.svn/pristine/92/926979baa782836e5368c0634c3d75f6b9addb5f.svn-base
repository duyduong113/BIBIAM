...
namespace ERPAPD.Controllers
{
    [Authorize]
    [RequireHttps]
    public class CRMFAQTopicController : CustomController
    {
        //
        // GET: /CRMFAQTopic/
        ...
        public ActionResult Read([DataSourceRequest]DataSourceRequest request)
        {
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            {
                ...
                if (asset.View)
                {
                    data = KendoApplyFilter.KendoData<CRM_FAQ_Topic>(request);
                }
                return Json(data);
            }
        }
        ...
        public ActionResult Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]IEnumerable<CRM_FAQ_Topic> items)
        {
            if (asset.Create)
            {
                if (items.Count() > 0)
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]IEnumerable<CRM_FAQ_Topic> items)
        {
            if (asset.Update)
            {
                if (items.Count() > 0)
            ...
            if (asset.Delete)
            {
                try
                {
                    using (IDbConnection dbConn = ERPAPD.Helpers.OrmliteConnection.openConn())
                    {
                        ...
                        {
                            var check = dbConn.FirstOrDefault<CRM_FAQ_Topic>("Id={0}", item);
                            dbConn.Delete(check);
                        }
                    }
                }
            ...
            if (asset.Export)
            {
                using (var dbConn = Helpers.OrmliteConnection.openConn())
                {
                    ...
                    if (request.Filters.Any())
                    {
                        ...
                        data = dbConn.Select<CRM_FAQ_Topic>(where).ToList();
                    }
                    else
                    {
                        data = dbConn.Select<CRM_FAQ_Topic>().ToList();
                    }
