...
{
    [Authorize]
    public class SurveyManagementController : CustomController
    {
        //
        ...
        public ActionResult Index()
        {
            ...
            //    dbConn.DropTables(typeof(DC_Survey_Management), typeof(DC_Survey_Management_Customer), typeof(DC_Survey_Management_Question));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(DC_Survey_Management_Customer), typeof(DC_Survey_Management_Question), typeof(DC_Survey_Management));
            //}
            //using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            //{
            //    OrmLiteConfig.DialectProvider.UseUnicode = true;
            //    dbConn.DropTables(typeof(DC_Survey_Management_Customer));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(DC_Survey_Management_Customer));
            //}
            //using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            //{
            //    OrmLiteConfig.DialectProvider.UseUnicode = true;
            //    dbConn.DropTables(typeof(DC_Survey_Management_Proceeded), typeof(DC_Survey_Management_Proceeded_Answer));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(DC_Survey_Management_Proceeded), typeof(DC_Survey_Management_Proceeded_Answer));
            //}
            //using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            //{
            //    OrmLiteConfig.DialectProvider.UseUnicode = true;
            //    dbConn.DropTables(typeof(DC_Survey_Management_User));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(DC_Survey_Management_User));
            //}
            //using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            //{
            //    OrmLiteConfig.DialectProvider.UseUnicode = true;
            //    dbConn.DropTables(typeof(DC_Survey_Management_Skip));
            //    const bool overwrite = false;
            //    dbConn.CreateTables(overwrite, typeof(DC_Survey_Management_Skip));
            //}
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                if (asset.View)
                {
                    ...
                    ViewBag.Category = dbConn.Select<DC_Survey_Category>().OrderBy(a => a.Id);
        ...
        public ActionResult SurveyManagement_Read([DataSourceRequest] DataSourceRequest request)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    return Json(KendoApplyFilter.KendoData<DC_Survey_Management>(request));
                }
            }
            else
            {
        ...
        public ActionResult SurveyManagement_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_Management> list)
        {
                ...
                if (asset.Create)
                {
                    try
                    {
                        if (list != null && ModelState.IsValid)
                        {
                            ...
                            {
                                var checkColumName = dbConn.Select<DC_Survey_Management>("[Title] = {0}", item.Title).FirstOrDefault();
        ...
        public ActionResult SurveyManagement_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_Management> list)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
        ...
        public ActionResult ActionUser_Read([DataSourceRequest] DataSourceRequest request, string SurveyManagementID)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    return Json(KendoApplyFilter.KendoData<DC_Survey_Management_User>(request, "SurveyManagementID=" + SurveyManagementID));
                }
            }
            else
            {
        ...
        public ActionResult Question_Read([DataSourceRequest] DataSourceRequest request, string SurveyManagementID)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    string query = @"SELECT q.Id, q.QuestionID,q.[Description],mq.SortOrder FROM DC_Survey_Management_Question mq
                                    LEFT JOIN DC_Survey_Question q
                                    ON mq.QuestionID = q.QuestionID
                                    WHERE mq.SurveyManagementID = " + SurveyManagementID;
                    return Json(KendoApplyFilter.KendoDataByQuery<DC_Survey_Management_Question>(request, query, ""));
                }
            }
            else
            {
        ...
        public ActionResult Customer_Read([DataSourceRequest] DataSourceRequest request, string SurveyManagementID)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    return Json(KendoApplyFilter.KendoData<DC_Survey_Management_Customer>(request, "SurveyManagementID=" + SurveyManagementID));
                }
            }
            else
            {
        ...
        public ActionResult QuestionResult_Read([DataSourceRequest] DataSourceRequest request, string SurveyManagementID)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    string query = @"SELECT q.Id,q.QuestionID,q.[Description],mq.SurveyManagementID,q.Type,mq.SortOrder FROM DC_Survey_Management_Question mq
                                    LEFT JOIN DC_Survey_Question q
                                    ON mq.QuestionID = q.QuestionID
                                    WHERE mq.SurveyManagementID = " + SurveyManagementID;
                    return Json(KendoApplyFilter.KendoDataByQuery<DC_Survey_Management_Question>(request, query, ""));
        ...
        public ActionResult SurveyListStatisticsDetail_Read([DataSourceRequest] DataSourceRequest request, string QuestionID, int ManagementID)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    ...
                    List<DC_Survey_Management> Result = new List<DC_Survey_Management>();
                    //lay cau hoi hien tai
                    var cQuestion = dbConn.FirstOrDefault<DC_Survey_Question>("QuestionID={0}", QuestionID);
                    //check neu la free text
                    if (cQuestion.Type == "Free text")
                    {
                        //lay tat ca cau tra loi ra.
                        var allFreetext = dbConn.Select<DC_Survey_Management_Proceeded>("QuestionID={0} AND SurveyManagementID={1}", QuestionID, ManagementID);
                        int index = 0;
                        foreach (var freetext in allFreetext)
                        {
                            DC_Survey_Management management = new DC_Survey_Management();
                    ...
                    else
                    {
                        ...
                        var listAnswer = dbConn.Select<DC_Survey_AnswerList>("QuestionID={0}", QuestionID);
                        //total so nguoi tra loi cau nay
                        double totalAnswer = dbConn.Count<DC_Survey_Management_Proceeded>(a => a.QuestionID == QuestionID && a.SurveyManagementID == ManagementID);
                        ...
                        foreach (var answer in listAnswer)
                        {
                            ...
                            management.Actual = (int)dbConn.Count<DC_Survey_Management_Proceeded>(a => a.AnswerID.Contains(answer.AnswerID));
                ...
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    if (Source == "SurveyCustomer")
                    {
                        string query = @"SELECT Id, CustomerID, Name AS Fullname,Phone,PhysicalID, Email
                                        FROM	DC_Survey_Management_Customer 
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        {
                            dbConn.Delete<DC_Survey_Management>(where: "Id={0} AND [Status] = N'Mới'".Params(item));
                        }
                        dbTrans.Commit();
                    }
                    catch (Exception ex)
        ...
        public ActionResult GetQuestionByCategory(string CategoryID)
        {
            ...
            var data = dbConn.Select<DC_Survey_Question>("Active =1 AND CategoryID = '" + CategoryID + "'");
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        string[] separators = { "@@" };
                        var listdata = data.Split(separators, StringSplitOptions.RemoveEmptyEntries);
                        var currentSuvey = dbConn.FirstOrDefault<DC_Survey_Management>("Id={0}", id);
                        ...
                        foreach (var item in listdata)
                        {
                            var checkexits = dbConn.FirstOrDefault<DC_Survey_Management_Question>("QuestionID= {0} AND SurveyManagementID= {1}", item, id);
                            if (checkexits != null)
                            {
        ...
        public ActionResult SaveQuestion(DC_Survey_Management_Question model)
        {
            if (asset.Update && ModelState.IsValid)
            {
                ...
                using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                {
                    ...
                    {
                        //lay survey hien tai
                        var currentSurvey = dbConn.FirstOrDefault<DC_Survey_Management>("Id={0}", model.SurveyManagementID);
                        ...
                        var currentQuestion = dbConn.Select<DC_Survey_Management_Question>("SurveyManagementID={0}", model.SurveyManagementID);
                        //check question neu la all
                        if (string.IsNullOrEmpty(model.QuestionID))
                        {
                            //lay tat ca question trong category
                            var questionlist = dbConn.Select<DC_Survey_Question>("CategoryID={0}", model.CategoryID);
                            ...
                            if (questionlist != null)
                            {
                                foreach (var question in questionlist)
                                {
                                    ...
                                    {
                                        var newQuestion = new DC_Survey_Management_Question();
        ...
        public ActionResult Customer_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_Management_Customer> list, int SurveyManagementID)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Create)
                {
                    try
                    {
                        var current = dbConn.FirstOrDefault<DC_Survey_Management>("Id=" + SurveyManagementID);
                        ...
                        if (list != null && ModelState.IsValid)
                        {
                            foreach (var item in list)
                            {
                                var checkColumName = dbConn.FirstOrDefault<DC_Survey_Management_Customer>("([Phone] = {0} OR ([PhysicalID]<>'' AND [PhysicalID]={1})) AND SurveyManagementID={2}", item.Phone, item.PhysicalID, SurveyManagementID);
                                if (checkColumName != null)
                                {
                                    dbTrans.Rollback();
        ...
        public ActionResult Customer_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<DC_Survey_Management_Customer> list, int SurveyManagementID)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
                    ...
                    try
                    {
                        if (list != null && ModelState.IsValid)
                        {
                            foreach (var item in list)
                            {
                                ...
                                var checkColumName = dbConn.FirstOrDefault<DC_Survey_Management_Customer>("([Phone] = {0} OR ([PhysicalID]<>'' AND [PhysicalID]={1})) AND SurveyManagementID={2} AND Id <>{3}", item.Phone, item.PhysicalID, SurveyManagementID, item.Id);
                                if (checkColumName != null)
                                {
                                    dbTrans.Rollback();
                                    ModelState.AddModelError("", "Khách hàng số điện thoại " + item.Phone + " có thông tin trùng với khách hàng khác trong survey này. ");
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        foreach (var item in listdata)
                        {
                            var checkexits = dbConn.FirstOrDefault<DC_Survey_Management_Customer>("Id = {0} AND SurveyManagementID= {1}", item, id);
                ...
                if (Request.Files["FileUpload"] != null && Request.Files["FileUpload"].ContentLength > 0)
                {
                    ...
                    if (fileExtension == ".xlsx")
                    {
                        ...
                        using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                        {
                            ...
                            FileInfo template = new FileInfo(Server.MapPath(@"~\ExportExcelFile\DC_SurveyCustomer.xlsx"));
                            ...
                            for (int i = 2; i <= totalRows; i++)
                            {
                                ...
                                try
                                {
                                    var check = string.IsNullOrWhiteSpace(Id) ? null : dbConn.Select<DC_Survey_Management_Customer>("Id =" + Id).FirstOrDefault();
                                    ...
                                    else
                                    {
                                        if (check != null)
                                        {
                                            ...
                                            var checkColumName = dbConn.FirstOrDefault<DC_Survey_Management_Customer>("[Phone] = {0}  AND SurveyManagementID={1} AND Id <>{2}", Phone, SurveyManagementID, Id);
                                            if (checkColumName != null)
                                            {
                                                eSheet.Cells[rownumber, 1].Value = Id;
                                                eSheet.Cells[rownumber, 2].Value = OrderID;
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                var data = new DC_Survey_Management_Customer();
                                                data.CustomerID = "";
                                                data.IsDone = false;
                                                data.Name = Name;
                                                data.Phone = Phone;
        ...
        public ActionResult ExportCustomer(string SurveyManagementID)
        {
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                if (asset.Export)
                {
                    ...
                    var data = KendoApplyFilter.KendoData<DC_Survey_Management_Customer>(request, "SurveyManagementID=" + SurveyManagementID);
                    IEnumerable datas = data.Data;

                    //using (ExcelPackage excelPkg = new ExcelPackage())
                    FileInfo fileInfo = new FileInfo(Server.MapPath(@"~\ExportExcelFile\DC_SurveyCustomer.xlsx"));
                    var excelPkg = new ExcelPackage(fileInfo);
                    ExcelWorksheet dataSheet = excelPkg.Workbook.Worksheets["DC_SurveyCustomer"];
                    int rowData = 1;
                    foreach (DC_Survey_Management_Customer item in datas)
                    {
                        int i = 1;
                        rowData++;
                    ...
                    string fileName = "DC_SurveyCustomer_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xlsx";
                ...
                else
                {
                    ...
                    return File("", //The binary data of the XLS file
                        ...
                        "DC_SurveyCustomer_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xls");     //Suggested file name in the "Save as" dialog which will be displayed to the end user
                }
        }
        [HttpPost]
        public ActionResult SaveSurvey(DC_Survey_Management_Proceeded model)
        {
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
            {
                if (ModelState.IsValid && asset.Update)
                {
                    try
                    {
                        ...
                        var currentSurvey = dbConn.FirstOrDefault<DC_Survey_Management>("Id=" + model.SurveyManagementID);
                        var write = new DC_Survey_Management_Proceeded();
                        string query = @"SELECT q.Id, q.CategoryID,q.Type, q.QuestionID,q.[Description] FROM DC_Survey_Management_Question mq
                                    LEFT JOIN DC_Survey_Question q
                                    ON mq.QuestionID = q.QuestionID
                                    WHERE mq.SurveyManagementID = " + model.SurveyManagementID + " Order By mq.SortOrder ASC";
                        var listQuestion = dbConn.Select<DC_Survey_Question>(query);
                        int count = 0;
                        foreach (var item in listQuestion)
                        {
                            if (item.Type == "Multi choice")
                            {
                                item.results = DC_Survey_Question.GetQuestionList(item.QuestionID);
                                ...
                                write.Answer = "";
                                var listAnswer = dbConn.Select<DC_Survey_AnswerList>("QuestionID={0}", write.QuestionID);
                                ...
                                foreach (var answer in item.results.Where(a => write.AnswerID.Contains(a.AnswerID)))
                                {
                                    ...
                                    {
                                        DC_Survey_Management_Proceeded_Answer pa = new DC_Survey_Management_Proceeded_Answer();
                            ...
                            else if (item.Type == "Single choice")
                            {
                                ...
                                var listAnswer = dbConn.FirstOrDefault<DC_Survey_AnswerList>("AnswerID={0}", write.AnswerID);
                                //save maping vao cau tra loi
                        ...
                        if (model.Source == "SurveyCustomer")
                            dbConn.Update<DC_Survey_Management_Customer>(set: "IsDone=1", where: "Phone='" + model.Phone + "'");
                        dbTrans.Commit();
                        return Json(new { success = true, });
        ...
        public ActionResult GetQuestionByManagementID(int ManagementID)
        {
            using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
            {
                ...
                {
                    //get current survey:
                    var management = dbConn.FirstOrDefault<DC_Survey_Management>("Id=" + ManagementID);
                    ...
                    if (management.StartDate < DateTime.Now && DateTime.Now < management.EndDate)
                    {
                        ...
                        var data = dbConn.Select<DC_Survey_Question>(query);
        ...
        public ActionResult LoadNextSurveyCustomer(int ManagementID)
        {
            using (var dbConn = OrmliteConnection.openConn())
            {
                var nextCustomer = dbConn.FirstOrDefault<DC_Survey_Management_Customer>("SELECT * FROM DC_Survey_Management_Customer WHERE SurveyManagementID = {0} AND IsDone=0 AND Id NOT IN (SELECT ManagementCustomerID FROM DC_Survey_Management_Skip WHERE SurveyManagementID = {0})", ManagementID);
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        foreach (var item in listdata)
                        {
                            var current = dbConn.FirstOrDefault<DC_Survey_Management>("Id=" + item);
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        foreach (var item in listdata)
                        {
                            var checkexits = dbConn.FirstOrDefault<DC_Survey_Management_User>("Id = {0} AND SurveyManagementID= {1}", item, id);
                            if (checkexits != null)
                            {
                                dbConn.Delete(checkexits);
                            }
        ...
        public ActionResult SaveActionUser(DC_Survey_Management_User model)
        {
            if (asset.Update && ModelState.IsValid)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                {
                    try
                    {
                        ...
                        var currentUserlist = dbConn.Select<DC_Survey_Management_User>("SurveyManagementID={0}", model.SurveyManagementID);
                        //check question neu la all
                        if (model.listUserName.Contains("Tất cả"))
                        {
                            ...
                            if (userlist != null)
                            {
                                foreach (var user in userlist)
                                {
                                    ...
                                    {
                                        var newQuestion = new DC_Survey_Management_User();
        ...
        public PartialViewResult PartialResultByCustomer(string ManagementID)
        {
            using (var dbConn = OrmliteConnection.openConn())
            {
                ...
                string query = @"SELECT mq.Id, q.QuestionID, q.[Description] FROM DC_Survey_Management_Question mq
                                    LEFT JOIN DC_Survey_Question q
                                    ON mq.QuestionID = q.QuestionID
                                    WHERE mq.SurveyManagementID = " + ManagementID + " ORDER BY mq.SortOrder";
                var listQuestion = dbConn.Select<DC_Survey_Management_Question>(query);
            ...
            using (var dbConn = OrmliteConnection.openConn())
            {
                ...
                string customerQuery = @"select * from (
	                                    SELECT  
		                                    ...
		                                    ,CustomerName
	                                    FROM (
		                                    SELECT Distinct Phone, CustomerName From DC_Survey_Management_Proceeded Where SurveyManagementID = " + ManagementID + @"
	                                    )AS result
                                    )a WHERE RowNum > " + (request.Page - 1) * request.PageSize + " AND RowNum<= " + request.Page * request.PageSize;
                var listCustomer = dbConn.Select<DC_Survey_Management_Proceeded>(customerQuery);
                result.Total = dbConn.Scalar<int>("SELECT COUNT(*) FROM (SELECT DISTINCT Phone,CustomerName FROM DC_Survey_Management_Proceeded WHERE SurveyManagementID=" + ManagementID + ") a");
                ...
                foreach (var customer in listCustomer)
                {
                    ...
                    var listAnswer = dbConn.Select<DC_Survey_Management_Proceeded>("Phone={0} AND CustomerName={1}", customer.Phone, customer.CustomerName);
                    foreach (var question in listQuestion)
                    {
        ...
        public ActionResult ExportData(string ManagementID)
        {
            if (asset.Export)
            {
                using (var dbConn = OrmliteConnection.openConn())
                {
                    ...
                    FROM DC_Survey_Management_Proceeded mp
                    LEFT JOIN DC_Survey_Management m ON mp.SurveyManagementID = m.Id
                    LEFT JOIN DC_Survey_Question q ON mp.QuestionID = q.QuestionID
                    ...
                    foreach (var item in data)
                    {
                        ...
                        else
                        {
                            ...
                            var listAnswer = dbConn.Select<DC_Survey_AnswerList>("QuestionID={0}", item.QuestionID);

                            //get list note of answer
                            var listNote = dbConn.Select<DC_Survey_Management_Proceeded_Answer>("ProceededID={0}", item.ProceededID);
        ...
        [HttpPost]
        public ActionResult SkipSurvey(DC_Survey_Management_Skip model)
        {
            if (asset.Update && ModelState.IsValid)
            {
                try
