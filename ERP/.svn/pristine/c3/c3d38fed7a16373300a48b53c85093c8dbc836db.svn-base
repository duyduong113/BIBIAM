...
{
    public class CRM_CS_PromotionInfoController : CustomController
    {
        ...
        public ActionResult Index()
        {
            ...
            //    dbConn.DropTables(typeof(CRM_Telesales_PromotionInfo), typeof(CRM_Telesales_PromotionInfo));
            //    const bool overwrite = true;
            //    dbConn.CreateTables(overwrite, typeof(CRM_Telesales_PromotionInfo), typeof(CRM_Telesales_PromotionInfo));
            //}
            if (asset.View)
            {
        ...
        public ActionResult Edit(int Id)
        {
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                ...
                if (asset.Update)
                {
                    ...
                    var hOld = dbConn.Select<CRM_Telesales_PromotionInfo>("SELECT * FROM CRM_Telesales_PromotionInfo Where ID = " + Id).FirstOrDefault();
                    if (hOld != null)
                    {
                        ViewData["listComp"] = dbConn.GetFirstColumn<string>("SELECT ShortName from Deca_Company");
                        return View(hOld);
        ...
        [ValidateInput(false)]
        public ActionResult Create(CRM_Telesales_PromotionInfo article, string filepath)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        [ValidateInput(false)]
        public ActionResult Edit(CRM_Telesales_PromotionInfo article, string filepath)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update && ModelState.IsValid)
                {
                    try
                    {
                        var CheckExit = dbConn.FirstOrDefault<CRM_Telesales_PromotionInfo>("ID =" + article.ID);
                        if (CheckExit == null)
                        {
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Delete)
                {
                    try
                    {
                        ...
                        var delete = new CRM_Telesales_PromotionInfo();
        ...
        public ActionResult DailyNew_Read([DataSourceRequest] DataSourceRequest request)
        {
            if (asset.View)
            {
                using (IDbConnection dbConn = Helpers.OrmliteConnection.openConn())
                {
                    return Json(KendoApplyFilter.KendoData<CRM_Telesales_PromotionInfo>(request));
                }
            }
            else
            {
        ...
        public ActionResult DailyNew_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<CRM_Telesales_PromotionInfo> list)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Create)
                {
                    try
                    {
                        ...
                        {
                            var data = new CRM_Telesales_PromotionInfo();
        ...
        public ActionResult DailyNew_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<CRM_Telesales_PromotionInfo> list)
        {
            using (var dbConn = Helpers.OrmliteConnection.openConn())
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
            ...
            using (var dbTrans = dbConn.OpenTransaction(IsolationLevel.ReadCommitted))
                if (asset.Update)
                {
                    try
                    {
                        ...
                        foreach (var item in listRowID)
                        {
                            dbConn.Update<CRM_Telesales_PromotionInfo>(set: "[IsActive]  = {0}".Params(false));
                        }
                        dbTrans.Commit();
                    }
                    catch (Exception e)
