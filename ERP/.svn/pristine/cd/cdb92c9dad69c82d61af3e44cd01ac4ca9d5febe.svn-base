...
{
    public class CRMCountryDefinitionController : CustomController
    {
        //
        ...
        public ActionResult Index()
        {
            if (asset.View)
            {
                ...
                ViewBag.listalias = khu_vuc.GetCRM_Location_Alias("1=1", "ten_khu_vuc ASC");
                return View();
            }
            else
            {
        ...
        {
            var data = new List<CRM_Location_Countries>();
            if (request.Filters.Any())
            {
                var where = KendoApplyFilter.ApplyFilter(request.Filters[0], "data.");
                data = CRM_Location_Countries.GetCRM_Location_Countries(where, "CountryID DESC");
            }
            else
            {
                data = CRM_Location_Countries.GetCRM_Location_Countries("1=1", "CountryID DESC");
            }
            return Json(data.ToDataSourceResult(request));
        }
        public ActionResult Country_Create([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<CRM_Location_Countries> listEx)
        {
            if (asset.Create)
            {
                try
                {
                    if (listEx != null)
                    {
                        foreach (var regis in listEx)
                        {
                            ...
                            string id = "";
                            var write = new CRM_Location_Countries();
                            var checkID = CRM_Location_Countries.GetCRM_Location_Countries("1=1", "").OrderByDescending(m => m.RowID).FirstOrDefault();
                            if (checkID != null)
                            {
                            ...
                            var check = CRM_Location_Countries.GetCRM_Location_Countries("1=1", "").Where(s => s.CountryName.Trim().ToLower() == regis.CountryName.Trim().ToLower() && s.AliasID == regis.AliasID && s.Active == regis.Active).FirstOrDefault();
                            if (check != null)
                            {
        ...
        public ActionResult Country_Update([DataSourceRequest] DataSourceRequest request, [Bind(Prefix = "models")]  IEnumerable<CRM_Location_Countries> listEx)
        {
            if (asset.Update)
            {
                try
                {
                    if (listEx != null)
                    {
                        foreach (var regis in listEx)
                        {
                            ...
                            var write = new CRM_Location_Countries();
                            var check = CRM_Location_Countries.GetCRM_Location_Countries("1=1", "").Where(s => s.CountryName.Trim().ToLower() == regis.CountryName.Trim().ToLower() && s.AliasID == regis.AliasID && s.Active == regis.Active);
                            if (check.Count() > 0)
                            {
                                ModelState.AddModelError("", "Country Name is exists.");
                                return Json(listEx.ToDataSourceResult(request, ModelState));
        ...
        public ActionResult DeleteCountry(string data)
        {
            if (asset.Delete)
            {
                try
                {
                    ...
                    {
                        var checkexist = CRM_Location_Region.GetCRM_Location_Regions("1=1", "").Where(s => s.CountryID == item);
                        if (checkexist.Count() > 0)
                        {
                            return Json(new { success = false, alert = "Country exist in Region" });
                        }
                        var check = new CRM_Location_Countries();
        ...
        public FileResult ExportExcel([DataSourceRequest]DataSourceRequest request)
        {
            if (asset.Export)
            {
                var country = CRM_Location_Countries.GetAllCRM_Location_Countries().OrderByDescending(s => s.CountryID).ToList();
                ...
                int rowData = 1;
                foreach (CRM_Location_Countries data in listData)
                {
                    int i = 1;
                    rowData++;
                ...
                if (Request.Files["FileUpload"] != null && Request.Files["FileUpload"].ContentLength > 0)
                {
                    ...
                    if (fileExtension == ".xlsx")
                    {
                        ...
                        for (int i = 2; i <= totalRows; i++)
                        {
                            ...
                            {
                                var write = new CRM_Location_Countries();
                                var checkCountryName = CRM_Location_Countries.GetCRM_Location_Countries("[CountryName] = N'" + countryName + "' AND [AliasID] = '" + id_Alias + "'", "").FirstOrDefault();
