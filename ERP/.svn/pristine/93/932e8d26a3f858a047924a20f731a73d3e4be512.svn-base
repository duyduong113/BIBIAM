...
{
    public class TypeOfBusinessController : CustomController
    {
                    ...
                    try
                    {
                        if (listItem != null)
                        {
                            foreach (var item in listItem)
                            {
                                ...
                                var checkCode = dbConn.SingleOrDefault<ERPAPD_MasterData_Customer>("Code ={0} AND Type ={1}", item.Code, "TypeOfBusiness");
                                if (checkCode != null)
                                {
                    ...
                    try
                    {
                        if (listItem != null)
                        {
                            foreach (var item in listItem)
                            {
                                ...
                                var checkCode = dbConn.SingleOrDefault<ERPAPD_MasterData_Customer>("Code ={0} AND Type ={1} AND ID <> {2}", item.Code, "TypeOfBusiness", item.ID);
                                if (checkCode != null)
                                {
                                    ModelState.AddModelError("", "Trùng mã !");
                                    return Json(listItem.ToDataSourceResult(request, ModelState));
            ...
            if (asset.Delete)
            {
                try
                {
                    using (IDbConnection dbConn = ERPAPD.Helpers.OrmliteConnection.openConn())
                    {
                        ...
                        {
                            var check = dbConn.FirstOrDefault<ERPAPD_MasterData_Customer>("ID={0} AND Type = {1}", item, "TypeOfBusiness");
                            dbConn.Delete(check);
                        }
                    }
                }
